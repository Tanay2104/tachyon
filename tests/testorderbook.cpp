...
protected:
    ...
    ClientRequest makeReq(ClientId cid, OrderId oid, Side side, Price price, Quantity qty, RequestType type = RequestType::New) {
        ClientRequest req;
        req.type = type;
        req.client_id = cid;
        req.time_stamp = current_time++; // Auto-increment for strict time priority testing
...
TEST_F(OrderBookTest, SingleOrderNoMatch) {
    ...
    EXPECT_EQ(book.size_asks(), 1); // Only counting one side based on implementation usually, or total?
                               // Code check: size() returns min(bids, asks) in the provided snippet?
                               // Actually, check code.txt: size() returns std::min(bids.size(), asks.size());
                               // This is a weird metric for a test, but let's stick to functional matching checks.
...
TEST_F(OrderBookTest, FullMatch) {
    ...
    EXPECT_EQ(trades[0].first.price, 100);
...
TEST_F(OrderBookTest, PricePriority_Bid) {
    ...
    auto b1 = makeReq(1, 100, Side::BID, 100, 10); book.add(b1);
    auto b2 = makeReq(2, 101, Side::BID, 101, 10); book.add(b2);
    auto b3 = makeReq(3, 102, Side::BID, 102, 10); book.add(b3);
...
TEST_F(OrderBookTest, PricePriority_Ask) {
    ...
    auto a1 = makeReq(1, 100, Side::ASK, 100, 10); book.add(a1);
    auto a2 = makeReq(2, 101, Side::ASK, 101, 10); book.add(a2);
    auto a3 = makeReq(3, 102, Side::ASK, 102, 10); book.add(a3);
...
TEST_F(OrderBookTest, PartialFill_AggressorRemains) {
    ...
    ASSERT_EQ(trades.size(), 1);
    EXPECT_EQ(trades[0].first.maker_order_id, 201); // The previous aggressor became the maker
...
TEST_F(OrderBookTest, PartialFill_RestingOrderRetainsPriority) {
    ...
    // If an order is partially filled, it must NOT lose its place in the queue (Time Priority).
    ...
    // Should match remaining 50 of A (because it still has priority over B), then 10 of B.
...
TEST_F(OrderBookTest, WalkingTheBook) {
    ...
    EXPECT_EQ(trades[1].first.price, 101);

    // 3. Clears 5 @ 102 (Partial)
    EXPECT_EQ(trades[2].first.maker_order_id, 12);
    EXPECT_EQ(trades[2].first.price, 102);
...
TEST_F(OrderBookTest, SelfTradePrevention) {
    ...
    // Should NOT trade.
    // Based on code reading: implementation skips matching but continues iterating.
    // The aggressor remains in the book (or logic might vary, but trade count must be 0).
...
TEST_F(OrderBookTest, SelfTradeSkip) {
    ...
    // Note: The remaining 10 quantity of the aggressor might be resting or rejected
    // depending on specific STP implementation, but the match verification is key here.
}
