...
static void BM_Flat_Lookup_Ideal(benchmark::State& state) {
  ...
  flat_hashmap<Key, Value48> map(N * 2);
  for (size_t i = 0; i < N; ++i) map.insert(keys[i], vals[i]);
...
static void BM_Flat_Remove(benchmark::State& state) {
  const size_t N = state.range(0);
  auto keys = make_keys(N);
  auto vals = make_values(N);
  ...
  for (auto _ : state) {
    ...
    for (size_t i = 0; i < N / 2; ++i) map.remove(keys[i]);
...
static void BM_Unordered_Remove(benchmark::State& state) {
  const size_t N = state.range(0);
  auto keys = make_keys(N);
  auto vals = make_values(N);
...
static void BM_Map_Remove(benchmark::State& state) {
  const size_t N = state.range(0);
  auto keys = make_keys(N);
  auto vals = make_values(N);
...
static void BM_Flat_Insert_After_Delete(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    for (size_t i = N; i < N + N / 2; ++i) map.insert(keys[i], vals[i]);
...
BENCHMARK(BM_Flat_Remove)->Arg(1024)->Arg(4096)->Arg(32768)->Arg(262144);
BENCHMARK(BM_Unordered_Remove)->Arg(1024)->Arg(4096)->Arg(32768)->Arg(262144);
BENCHMARK(BM_Map_Remove)->Arg(1024)->Arg(4096)->Arg(32768)->Arg(262144);
